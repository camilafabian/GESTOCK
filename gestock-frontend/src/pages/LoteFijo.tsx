...
const LoteFijo: React.FC = () => {
  ...
  const loadArticulos = async () => {
    try {
      ...
      // Filtrar solo artículos con modelo "Lote Fijo"
      const articulosLoteFijo = response.data.filter((articulo) =>
        articulo.modelo?.nombreModelo.toLowerCase().includes("lote fijo")
      );
      setArticulos(articulosLoteFijo);
    } catch (err) {
    ...
    return (
      <div className="loading">
        <div className="text-lg">
          Cargando artículos con modelo Lote Fijo...
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Modelo Lote Fijo</h1>
        <p className="text-gray-600 mt-1">
          Artículos gestionados con el modelo de inventario de Lote Fijo
        </p>
      </div>
      ...
      <div className="card bg-green-50 border-green-200">
        <div className="flex items-start space-x-3">
          <div className="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center flex-shrink-0">
            <span className="text-green-600 font-bold">LF</span>
          </div>
          <div>
            <h3 className="font-semibold text-green-900">Modelo Lote Fijo</h3>
            <p className="text-sm text-green-700 mt-1">
              En este modelo se calcula un lote óptimo de compra y un punto de
              pedido. Cuando el stock alcanza el punto de pedido, se genera
              automáticamente una orden de compra por el lote óptimo calculado.
            </p>
          </div>
        </div>
      </div>
      ...
      {articulos.length > 0 && (
        <div className="flex justify-between items-center">
          <div className="text-sm text-gray-600">
            ...
            {articulos.length !== 1 ? "s" : ""} con modelo Lote Fijo
          </div>
        </div>
      )}
